  ...
  if (_indirect)
    {
      ...
      unsigned l2_pages = div_ceil(entries, entries_per_page);
      unsigned l1_pages = div_ceil(l2_pages, _page_size / L1_entry::Size);
      size = l1_pages * _page_size;
    }
  else
    {
      size = cxx::ceil_lsb(size, log2(_page_size));
    }
...
Gic_its::Device::setup_itt()
{
  ...
  _itt.make_coherent();
  _its.send_cmd(Cmd::mapd(_id, _itt.phys_addr(), log2(_its.num_lpis())));
  return true;
}
