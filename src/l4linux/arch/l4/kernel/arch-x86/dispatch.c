...
#ifdef CONFIG_X86_64
	do_syscall_64(regsp, syscall);
#else
	do_int80_syscall_32(regsp);
#endif
...
static int read_value_1(u8 *value, unsigned long addr,
                        struct pt_regs *regs)
{
	if (user_mode(regs))
		return get_user(*value, (u8 *)addr);
	*value = *(u8 *)addr;
	return 0;
}
 ...
 */
static int l4x_port_emulation(struct pt_regs *regs)
{
	...
	if (read_value_1(&insn, regs->ip, regs))
		return 0; /* Memory could not be accessed */

	if (insn == 0x66) {
		operand_size_prefix = 1;
		ioffset = 1;
		if (read_value_1(&insn, regs->ip + ioffset, regs))
			return 0; /* Memory could not be accessed */
	}
	...
	if (insn & 8) {
		...
		port = 0;
		if (read_value_1((u8 *)&port, regs->ip + ioffset + 1, regs))
                                         ...
                                         unsigned long pfa)
{
	...
	switch (trapno) {
		case X86_TRAP_MF:
			exc_coprocessor_error(regs);
			break;
		case X86_TRAP_XF:
			exc_simd_coprocessor_error(regs);
			break;
		case X86_TRAP_NM:
		...
		case X86_TRAP_PF:
			...
			exc_general_protection(regs, err);
			break;
		default:
			goto unknown_fault;
	}
