...
#ifdef CONFIG_L4
			if (which == FS)
				L4X_SEG_CUR(fs_base) = next_base;
			else
				L4X_SEG_CUR(gs_base) = next_base;
#else /* L4 */
...
__switch_to(struct task_struct *prev_p, struct task_struct *next_p)
{
	...
	l4x_stack_struct_get((unsigned long)next_p->stack)->utcb
		= l4x_stack_struct_get((unsigned long)prev_p->stack)->utcb;
#endif
