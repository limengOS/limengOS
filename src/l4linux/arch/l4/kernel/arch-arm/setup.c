...
#ifdef CONFIG_L4
		unsigned int mmfr0 = l4lx_kinfo->platform_info.arch.cpuinfo.ID_MMFR[0];
#else
 ...
 * L4Lx: we just define the two functions where we define
 *       the two structures (the only two we have).
 */
...
static int __init early_mem(char *p)
{
	...
	/* L4x: we handle memory in another place */
	return 0;
	/*
...
static void __init setup_arch_l4x_mem(void)
{
	...
	BUG_ON((unsigned long)_text & ((1UL << 20) - 1));
	BUG_ON(((unsigned long)_end - (unsigned long)_text)
		& ((1UL << 20) - 1));
...
#ifdef CONFIG_OF
	atags_vaddr = l4x_load_dtb_virt(boot_command_line, 0x100);
	__atags_pointer = virt_to_phys(atags_vaddr);
#endif
#else /* L4 */
	if (__atags_pointer)
