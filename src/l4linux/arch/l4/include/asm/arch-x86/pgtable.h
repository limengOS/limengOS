...
#ifdef CONFIG_L4
/* L4Linux has no PGD entries of the kernel in user page tables */
...
#ifdef CONFIG_L4
	__l4x_set_pmd(mm, addr, pmdp, pmd);
#else
	set_pmd(pmdp, pmd);
#endif
}
...
#ifdef CONFIG_L4
	__l4x_set_pud(mm, addr, pudp, pud);
#else
...
void l4x_pud_update(struct mm_struct *mm, unsigned long addr, pud_t *pudp);
#endif /* L4 */
