...
static int l4x_kvm_map(struct kvm_vcpu *vcpu, gpa_t v, int write,
                       int level, gfn_t gfn, kvm_pfn_t pfn)
{
	l4_fpage_t fp;

	if (level != PT_PAGE_TABLE_LEVEL) {
		pr_err("l4x-kvm: ERROR: level: %d != %d\n",
		       PT_PAGE_TABLE_LEVEL, level);
		return -ENOMEM;
	}

	fp = l4_fpage((unsigned long)pfn_to_kaddr(pfn), L4_LOG2_PAGESIZE,
                      write ? L4_FPAGE_RWX : L4_FPAGE_RX);

	if (L4XV_FN_e(l4_task_map(vcpu->kvm->arch.l4vmcap,
	                          L4RE_THIS_TASK_CAP, fp,
	                          l4_map_control(v & PAGE_MASK,
	                                         0, L4_MAP_ITEM_MAP)))) {
		pr_err("l4x-kvm: map failed");
		return -ENOMEM;
	}
...
static void l4x_kvm_unmap(struct kvm *kvm, u64 *spte, gfn_t gfn)
{
	l4_fpage_t fp;
	...
	// TODO restricted unmap
	fp = l4_fpage(gfn << PAGE_SHIFT, L4_LOG2_PAGESIZE, L4_FPAGE_RWX);
	if (L4XV_FN_e(l4_task_unmap(kvm->arch.l4vmcap, fp, L4_FP_ALL_SPACES)))
		pr_err("l4x-kvm: unmap failed gfn=%llx\n", gfn);
}

static void l4x_kvm_set_spte_ro(u64 spte)
{
	kvm_pfn_t pfn;
	l4_fpage_t fp;
	unsigned error;
	...
	fp = l4_fpage((unsigned long)pfn_to_kaddr(pfn),
	              is_large_pte(spte) ? L4_LOG2_SUPERPAGESIZE : L4_LOG2_PAGESIZE,
	              L4_FPAGE_W);

	/* This will also remove the write bit from Qemu. However, L4linux will
	 * page it back in transparently if needed. */
	error = L4XV_FN_e(l4_task_unmap(L4RE_THIS_TASK_CAP, fp,
	                                L4_FP_OTHER_SPACES));

	if (error)
		pr_err("l4x-kvm: unmap failed\n");
}
#endif /* L4 */
...
#ifdef CONFIG_L4
		// case should not be taken on L4
		BUG();
#endif
...
#ifdef CONFIG_L4
	if (!(rmap_head->val & 1)) // see pte_list_remove
		l4x_kvm_unmap(kvm, spte, gfn);
#endif /* L4 */
