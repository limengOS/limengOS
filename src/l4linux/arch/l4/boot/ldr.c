...
L4RE_ELF_AUX_ELEM_T(l4re_elf_aux_mword_t, _stack_size,
                    L4RE_ELF_AUX_T_STACK_SIZE, 0x2000);
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < ehdr->e_phnum; ++i) {
		...
		ElfW(Phdr) *ph = (ElfW(Phdr)*)((char *)l4util_elf_phdr(ehdr)
		                               + i * ehdr->e_phentsize);
		...
		if (ph->p_filesz < ph->p_memsz) {
			...
			r = l4re_ma_alloc(l4_round_page(ph->p_memsz), ds,
		...
		r = l4re_ma_alloc(l4_round_page(ph->p_memsz), new_ds,
		                  L4RE_MA_CONTINUOUS | L4RE_MA_PINNED);
		if (r) {
			printf("lxldr: could not allocate memory: %d\n", r);
			return 1;
