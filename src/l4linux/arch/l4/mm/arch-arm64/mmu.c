...
#ifdef CONFIG_L4
u64 __section(".mmuoff.data.write") vabits_actual = CONFIG_ARM64_VA_BITS;
#else /* L4 */
...
#else /* L4 */
	printk("L4x: update_mapping_prot(%llx, %lx, %llx, ...)\n",
	       phys, virt, size);
#endif /* L4 */
}
...
#ifdef CONFIG_L4
	(void)vmlinux_inittext;
	(void)vmlinux_initdata;
	(void)vmlinux_data;
#else /* L4 */
...
#ifdef CONFIG_L4
			l4lx_memory_map_virtual_range(addr & ~(PMD_SIZE - 1), PMD_SIZE, __pa(p), 1);
#endif /* L4 */
		} else
