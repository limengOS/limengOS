...
enum {
	VMEM_BITMAP_BITS = (L4X_VMEMMAP_END - VMEMMAP_START) / PMD_SIZE,
	VMEM_BITMAP_LONGS = BITS_TO_LONGS(VMEM_BITMAP_BITS),
};

static unsigned long __meminitdata vmembitmap[VMEM_BITMAP_LONGS];
                               ...
                               struct vmem_altmap *altmap)
{
	...
	for (addr = start; addr < end; addr = next) {
		unsigned long offset = addr - VMEMMAP_START;
		next = pmd_addr_end(addr, end);
		if (!test_bit(offset / PMD_SIZE, vmembitmap)) {
			int err;
			void *p;
			...
			err = l4lx_memory_map_virtual_range(addr & ~(PMD_SIZE - 1),
			                                    PMD_SIZE, __pa(p), 1);
			if (err)
				pr_err("%s: error\n", __func__);
			else
				set_bit(offset / PMD_SIZE, vmembitmap);

			if (p_end != p || node_start != node) {
				if (p_start)
					pr_debug(" [%lx-%lx] PMD -> [%p-%p] on "
					         "node %d\n",
