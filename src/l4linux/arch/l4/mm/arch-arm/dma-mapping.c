...
static void l4x_map_pages(unsigned long dst, unsigned long src, size_t sz)
{
	l4x_v2p_add_item(virt_to_phys((void *)src), (void *)dst, sz);

	while (sz > 0) {
		l4x_map_self(src, dst, L4_FPAGE_BUFFERABLE << 4);
		dst += PAGE_SIZE;
		src += PAGE_SIZE;
		sz  -= PAGE_SIZE;
	}
...
static void l4x_unmap_pages(unsigned long a, size_t sz)
{
	unsigned r = l4x_v2p_del_item((void *)a);
	WARN_ON(r != sz);
	while (sz > 0) {
		l4x_unmap_self(a);
		a  += PAGE_SIZE;
...
{
	struct vm_struct *v;
	if ((v = find_vm_area((void *)a)) && (v->flags & VM_DMA_COHERENT))
		return v->size;
	return 0;
}

static void *
l4x_dma_alloc_remap(struct page *page, size_t size, pgprot_t prot,
                    const void *caller)
{
	void *addr = dma_common_contiguous_remap(page, size, prot, caller);
	l4x_map_pages((unsigned long)addr, page_to_pfn(page) << PAGE_SHIFT,
	              size);
	return addr;
}

static void l4x_dma_free_remap(void *cpu_addr, size_t size)
{
	l4x_unmap_pages((unsigned long)cpu_addr, size);
				 ...
				 const void *caller, bool want_vaddr)
{
	...
	ptr = l4x_dma_alloc_remap(page, size, prot, caller);
	if (!ptr) {
				   ...
				   void *cpu_addr, size_t size, bool want_vaddr)
{
	if (want_vaddr) {
		if (PageHighMem(page))
			l4x_dma_free_remap(cpu_addr, size);
		else
...
{
	if (args->want_vaddr)
		l4x_dma_free_remap(args->cpu_addr, args->size);
...
static struct gen_pool *l4x_dmapool_mem_pool;

static int __init l4x_dmapool_mem_init(void)
{
	l4x_dmapool_mem_pool = gen_pool_create(12, -1);
	if (!l4x_dmapool_mem_pool) {
		printk(KERN_ERR "Failed to create memory pool for DMA memory\n");
		return 1;
	}
	return 0;
}
core_initcall(l4x_dmapool_mem_init);

int l4x_dmapool_mem_add(unsigned long virt, unsigned long phys, size_t size)
{
	int r = gen_pool_add_virt(l4x_dmapool_mem_pool, virt, phys, size, -1);
	if (r < 0)
		return r;

	if (0)
		printk("l4x_dmapool_mem_add: v=%08lx p=%08lx sz=%zd\n",
		       virt, phys, size);

	l4x_v2p_add_item(phys, (void *)virt, size);
...
{
	return    (phys_addr_t)-1 != gen_pool_virt_to_phys(l4x_dmapool_mem_pool,
	                                                   va)
	       && (phys_addr_t)-1 != gen_pool_virt_to_phys(l4x_dmapool_mem_pool,
	                                                   va + sz - 1);
}
EXPORT_SYMBOL(l4x_dmapool_is_in_virt_dma_space);

static unsigned long l4x_dmapool_mem_alloc(size_t size, dma_addr_t *dma)
{
	unsigned long va;
	dma_addr_t phys = 0;
	...
	va = gen_pool_alloc(l4x_dmapool_mem_pool, size);
	if (!va) {
		printk("l4x-dma-alloc: size=%zd avail=%zd/%zd\n",
		       size, gen_pool_avail(l4x_dmapool_mem_pool),
		       gen_pool_size(l4x_dmapool_mem_pool));
		return 0;
	}

	phys = gen_pool_virt_to_phys(l4x_dmapool_mem_pool, va);
	if (phys == (phys_addr_t)-1) {
		gen_pool_free(l4x_dmapool_mem_pool, va, size);
		return 0;
	}
...
static void l4x_dmapool_mem_free(unsigned long addr, size_t size)
{
	if (0)
		printk("l4x-dma-free: addr=%lx size=%zd\n", addr, size);
	gen_pool_free(l4x_dmapool_mem_pool, addr, size);
}
#endif /* L4_DMAPOOL */
...
#ifdef CONFIG_L4_DMAPOOL
	return (void *)l4x_dmapool_mem_alloc(size, handle);
#endif
...
#ifdef CONFIG_L4_DMAPOOL
	l4x_dmapool_mem_free((unsigned long)cpu_addr, size);
	return;
#endif
