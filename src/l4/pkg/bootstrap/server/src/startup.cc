...
bool arm64_is_el2_kernel;
bool arm64_do_syscall_patching;
#endif
  ...
  do
    {
      ...
      l4util_kip_for_each_feature(s)
        if (!strcmp(s, "arm:hyp"))
          {
            arm64_is_el2_kernel = true;
            break;
          }
    }
  while (0);
  arm64_do_syscall_patching = true;
#endif
...
#if defined(ARCH_arm64)
  if (arm64_do_syscall_patching && (section_type & EXEC_SECTYPE_EXECUTE))
    {
      l4_uint32_t *end = (l4_uint32_t *)((char *)dst + file_size);
      for (l4_uint32_t *cp = (l4_uint32_t *)dst; cp < end; ++cp)
        if (*cp == 0xd4000001 && arm64_is_el2_kernel) // svc #0
          {
            if (Verbose_load)
        ...
        else if (*cp == 0xd4000002 && !arm64_is_el2_kernel) // hvc #0
          {
            if (Verbose_load)
