...
EXTERN_C void *xmit_thread_fn(void *data)
{
	...
	while(true)
	{
		...
		sk = alloc_skb_buffer(size, &dat);

		int err = chunk->next_copy_out(dat, &size);
		assert(err == 0);

		if (session->debug()) {
			packet_analyze(dat, size);
		}

//		std::cout << "xmit " << (void*)skb->dat << " " << size << "\n";
		// try to deliver locally
		unsigned local = packet_deliver(dat, size, session->dev()->name(), static_cast<unsigned>(true));

		if (!local || Ankh::Util::is_broadcast_mac(dat)) {
			err = session->dev()->transmit(sk, size);
			chunk->notify_done();
		} else
		    free_skb_buffer(sk);
