...
#include <l4/dde/linux32/dde26_net.h>
...
#else
char const *mck_netdev_name(void *netdev)
{ return netdev_name(netdev); }
#endif
...
char * alloc_skb_buffer(unsigned size, char ** data)
{
...
void free_skb_buffer(char *sk)
{
    kfree_skb((struct sk_buff *)sk);
}
...
#else
		err = dev_queue_xmit(skb);                              //MMMadd need check correct; CONFIG_COMPAT_NET_DEV_OPS
		//err = ND(netdev)->netdev_ops->ndo_start_xmit(skb, ND(netdev));      //MMMadd need check correct; CONFIG_COMPAT_NET_DEV_OPS
#endif
    }
