...
Msi_irq_pin::msi_info(Msi_src *src, l4_icu_msi_info_t *info)
{
  ...
  // TODO: Architecture/Platform specific formatting of the MSI source info
  #if defined(ARCH_x86) || defined(ARCH_amd64)
    // Layout equivalent to layout expected by kernel (Irte)
  #elif defined(ARCH_arm) || defined(ARCH_arm64)
    Device::Msi_src_info src_info = si;
    if (src_info.mode() == Device::Msi_src_info::Verify_requester_id)
      si = src_info.sid();
    else if (src_info.mode() == Device::Msi_src_info::Verify_bus_range)
      // TODO: Raise error?!
      si = src_info.start_bus() << 8;
  #endif
