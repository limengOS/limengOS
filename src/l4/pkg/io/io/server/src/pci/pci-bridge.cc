...
private:
  struct Secondary_msi_src : Io_irq_pin::Msi_src
  {
    ...
    l4_uint64_t get_src_info(Msi_mgr *mgr) override
    {
      ...
      Device::Msi_src_info info(0);
      info.mode() = Device::Msi_src_info::Verify_bus_range;
      info.start_bus() = secondary;
      // TODO: Use subordinate to calculate end_bus?
      info.end_bus() = secondary;
      return info.v;
    }
  };
