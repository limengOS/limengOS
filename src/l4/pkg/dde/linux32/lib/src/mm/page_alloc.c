...
extern void * ddekit_simple_malloc(unsigned size);
#endif
...
#else
    if ((page = ddekit_alloc_pages(GFP_KERNEL, order)) != NULL) {
		rmv_page_order(page);
		return page;
	}
#endif
...
#else
		zone->wait_table = (wait_queue_head_t *)
			ddekit_simple_malloc(alloc_size);
#endif
	} else {
		/*
				...
				struct zone *zone, unsigned long zonesize)
{
	...
	if (usemapsize)
		...
		zone->pageblock_flags = alloc_bootmem_node_nopanic(pgdat,
								   usemapsize);
#else
		zone->pageblock_flags = ddekit_simple_malloc(usemapsize);
#endif
}
#else
...
#else
	pfn = ((unsigned long) page_address(page)) >> PAGE_SHIFT;
#endif
