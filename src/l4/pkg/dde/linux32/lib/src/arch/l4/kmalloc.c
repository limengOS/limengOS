...
static bootmem_data_t contig_bootmem_data[MAX_NUMNODES];
struct pglist_data contig_page_data = { .bdata = &contig_bootmem_data[0] };
                         ...
                         dma_addr_t *dma_handle, gfp_t flag)
{
	...
	if (ret != NULL) {
		memset(ret, 0, size);
		*dma_handle = (dma_addr_t)virt_to_bus(ret);
	}
	return ret;
}
...
void dma_outer_inv_range(phys_addr_t start, phys_addr_t end)
{
    l4_cache_inv_data(start, end);
}

void dma_outer_clean_range(phys_addr_t start, phys_addr_t end)
{
    l4_cache_clean_data(start, end);
}
