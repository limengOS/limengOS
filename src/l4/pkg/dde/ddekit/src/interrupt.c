...
static struct
{
	...
	ddekit_sem_t     *intsem;     /* int ctrl semaphore */
...
static void intloop(void *arg)
{
	...
	unsigned long tid = l4_debugger_global_id(pthread_l4_cap(pthread_self()));
    
	if (tid != ~0UL)
	    printf("Thread 0x%lx for IRQ %d\n", tid, params->irq);
	else
	    printf("Thread for IRQ %d\n", params->irq);
                                         ...
                                         void(*handler)(void *), void *priv)
{
	...
	thread = ddekit_thread_create(intloop, params, thread_name, DDEKIT_IRQ_PRIO + (MAX_INTERRUPTS - irq));
	if (!thread) {
	...
	ddekit_irq_ctrl[irq].intsem     = ddekit_sem_init(1);
...
void ddekit_interrupt_detach(int irq)
{
	...
	ddekit_sem_deinit(ddekit_irq_ctrl[irq].intsem);
...
{
	if (!l4_is_invalid_cap(ddekit_irq_ctrl[irq].irq_cap)) {
	    ddekit_sem_down(ddekit_irq_ctrl[irq].intsem);
		--ddekit_irq_ctrl[irq].handle_irq;
		ddekit_sem_up(ddekit_irq_ctrl[irq].intsem);
	}
}
