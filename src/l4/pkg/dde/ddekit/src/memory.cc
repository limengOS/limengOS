...
static L4::Cap<L4Re::Dma_space> dma __attribute__((init_priority(INIT_PRIO_DDEKIT_SLAB_DMA)));

#include <l4/sys/kdebug.h>
static void *ddekit_slab_allocate_new_region(long size, long mem_flags,
                                             L4Re::Rm::F::Flags attach_flags)
{
	void * ret = NULL;
	long   err = 0;

    if (!dma.is_valid()) {      //at the first time
        dma = L4Re::Util::cap_alloc.alloc<L4Re::Dma_space>();
        l4_error(L4Re::Env::env()->user_factory()->create(dma));
        dma->associate(L4::Ipc::Cap<L4::Task>(), L4Re::Dma_space::Phys_space);
    }
...
#if DEBUG
	ddekit_printf("\033[33mattach(ptr %p, size %d, flags %lx)\033[0m\n", ret, size, attach_flags | L4Re::Rm::F::Search_addr);
#endif
	err = L4Re::Env::env()->rm()->attach(&ret, size, 
                                          attach_flags | L4Re::Rm::F::Search_addr,
                                             L4::Ipc::make_cap(ds, (attach_flags & L4Re::Rm::F::W)
                                                                   ? L4_CAP_FPAGE_RW
                                                                   : L4_CAP_FPAGE_RO),
    ...
    err = dma->map(L4::Ipc::make_cap_rw(ds), 0, &psize, L4Re::Dma_space::Attributes::None, L4Re::Dma_space::Bidirectional, &phys);
#if DEBUG
	ddekit_printf("\033[33mphys addr @ %p, err %d\033[0m\n", phys, err);
#endif

	if (!err) // got pinned memory
		ddekit_pgtab_set_region_with_size(ret, (ddekit_addr_t)phys, psize, 0);
out:
	return ret;
}
...
//	ddekit_printf("detach %d %lx\n", err, ds.cap());
	...
	l4_addr_t phys = ddekit_pgtab_clear_region(addr, 0);
	if (phys)
        dma->unmap(phys, 0, L4Re::Dma_space::No_sync, L4Re::Dma_space::Direction::None);
...
static inline struct ddekit_pcache *_try_from_cache(int contig)
{
    ...
    /* MMMadd fix old issue: should use struct ddekit_pcache **the_cache */
...
EXTERN_C L4_CV void *_slab_grow(l4slab_cache_t *cache, void **data)
{
	...
	if (head) {
		...
		L4Re::Rm::F::Flags att_flags = L4Re::Rm::F::Eager_map | L4Re::Rm::F::RW;
