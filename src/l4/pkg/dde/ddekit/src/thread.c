...
#define DDEKIT_USE_KMEM_CACHE

#ifndef DDEKIT_USE_KMEM_CACHE
...
static struct kmem_cache  *ddekit_stack_slab = NULL;
#endif
...
#else
	stack  = kmem_cache_alloc(ddekit_stack_slab, GFP_KERNEL);
#endif
	if (stack == NULL)
...
#else
	kmem_cache_free(ddekit_stack_slab ,td->stack);
#endif
...
#else
	ddekit_stack_slab = kmem_cache_create("ddekit_thread_stack", 
                            DDEKIT_THREAD_STACK_SIZE, 0,
